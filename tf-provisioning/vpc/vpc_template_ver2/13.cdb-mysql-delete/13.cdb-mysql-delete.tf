...
resource "null_resource" "cdb-mysql-delete" {
    provisioner "local-exec" {
        ...
        command = <<EOF
CDB_NUM=$(${var.path_module}/cdb_mysql.sh GET "/vmysql/v2/getCloudMysqlInstanceList?vpcNo=${data.ncloud_vpc.vpc.id}&subnetNo=${data.ncloud_subnet.vpc_priv_subnet.id}&cloudMysqlServiceName=${var.cdbmysql_name}&responseFormatType=json" |grep cloudMysqlInstanceNo |cut -d "\"" -f4)
